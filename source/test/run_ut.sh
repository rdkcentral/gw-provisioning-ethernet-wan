...
log "INFO" "Running make for Gw_Prov_Ethwan_gtest.bin..."
if make -C source/test/Gw_Prov_EthwanTest; then
...
if [ -f "./source/test/Gw_Prov_EthwanTest/Gw_Prov_Ethwan_gtest.bin" ]; then
    log "INFO" "Running Gw_Prov_Ethwan_gtest.bin"
    ./source/test/Gw_Prov_EthwanTest/Gw_Prov_Ethwan_gtest.bin
    log "INFO" "Completed Test Execution"
else
    log "ERROR" "Gw_Prov_Ethwan_gtest.bin does not exist, cannot run tests"
    exit 1
fi
